...
{
  GstCaps* _cretval;
  const(char)* _codecsField = codecsField.toCString!(No.Malloc, No.Nullable);
...
{
  GList* _cretval;
  const(char)* _categoryname = categoryname.toCString!(No.Malloc, Yes.Nullable);
...
{
  bool _retval;
  const(char)* _codecTag = codecTag.toCString!(No.Malloc, Yes.Nullable);
