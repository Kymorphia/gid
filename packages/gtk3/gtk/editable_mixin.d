...
template EditableT()
{
  ...
  */
  override void insertText(string newText, ref int position)
  {
    int _newTextLength;
    if (newText)
      _newTextLength = cast(int)newText.length;

    auto _newText = newText.ptr ? cast(const(char)*)newText.ptr : [char.init].ptr;
    gtk_editable_insert_text(cast(GtkEditable*)this._cPtr, _newText, _newTextLength, cast(int*)&position);
  }
  ...
  /**
      ...
      Params:
        callback = signal callback delegate or function to connect
          ...
          $(D void callback(string newText, ref int position, gtk.editable.Editable editable))
  
          `newText` the new text to insert (optional)
  ...
  */
  ...
  && Parameters!T.length < 4)
  {
    extern(C) void _cmarshal(GClosure* _closure, GValue* _returnValue, uint _nParams, const(GValue)* _paramVals, void* _invocHint, void* _marshalData)
    {
      ...
      auto _dClosure = cast(DGClosure!T*)_closure;
      Tuple!(Parameters!T) _paramTuple;
      auto newTextLength = getVal!(int)(&_paramVals[2]);
