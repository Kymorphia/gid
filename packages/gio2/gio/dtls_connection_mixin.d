...
template DtlsConnectionT()
{
  ...
  */
  ...
  /**
      ...
      Throws: [ErrorWrap]
  */
  override bool getChannelBindingData(gio.types.TlsChannelBindingType type, ref ubyte[] data)
  {
    bool _retval;
    auto _data = gByteArrayFromD(data);
    scope(failure) containerFree!(GByteArray*, ubyte, GidOwnership.None)(_data);
    GError *_err;
    _retval = cast(bool)g_dtls_connection_get_channel_binding_data(cast(GDtlsConnection*)this._cPtr, type, _data, &_err);
    if (_err)
      throw new ErrorWrap(_err);
